...
const ACCESS_EXPIRES_IN = "10s"; // ИЗМЕНЕНО ДЛЯ ТЕСТА: Токен живет всего 10 секунд
const REFRESH_EXPIRES_IN = "2m"; // ИЗМЕНЕНО ДЛЯ ТЕСТА: Рефреш токен живет 2 минуты
...
app.post("/api/auth/refresh", (req, res) => {
  // Читаем refreshToken из HttpOnly cookie (приоритет) или из заголовка Authorization
  ...
  if (!refreshToken) {
    return res.status(400).json({ error: "Refresh token not found in cookie or Authorization header" });
  }
