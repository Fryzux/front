...
// users: { id, email, first_name, last_name, password, role, isBlocked }
let users = [];
...
app.post("/api/auth/register", async (req, res) => {
  ...
  // Разрешаем роли user, seller, admin при регистрации (для тестирования)
  // В реальном приложении роль admin/seller назначалась бы только администратором
...
// GET /api/auth/me — Пользователь (любая роль)
...
app.delete("/api/users/:id", authMiddleware, roleMiddleware(["admin"]), (req, res) => {
  ...
  res.json({ message: "User blocked successfully", id: req.params.id });
});

// Разблокировать пользователя — Администратор
app.patch("/api/users/:id/unblock", authMiddleware, roleMiddleware(["admin"]), (req, res) => {
  const userIndex = users.findIndex(u => u.id === req.params.id);
  if (userIndex === -1) return res.status(404).json({ error: "User not found" });

  users[userIndex].isBlocked = false;
  res.json({ message: "User unblocked successfully", id: req.params.id });
});
...
// GET /api/products — Пользователь (все авторизованные)
app.get("/api/products", authMiddleware, (req, res) => {
  res.status(200).json(products);
});

// GET /api/products/:id — Пользователь (все авторизованные)
...
// POST /api/products — Продавец и Администратор
...
// PUT /api/products/:id — Продавец и Администратор
